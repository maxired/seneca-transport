...
describe('transport', function () {
  ...
  it('happy-pin-tcp', function (fin) {
    test.foo_pintest('..', require, fin, 'tcp')
  })
  ...
  it('happy-pin-web', function (fin) {
    test.foo_pintest('..', require, fin, 'web')
  })
  ...
  it('own-message', function (fin) {
    ...
    function do_type (type, fin) {
      ...
      senecaA.ready(function () {
        senecaB.ready(function () {
          senecaA.act('a:1', function (err, out) {
            if (err) return fin(err)
            assert.equal(1, out.aa)
          })

          senecaA.act('b:1', function (err, out) {
            if (err) return fin(err)
            assert.equal(1, out.bb)
          })

          senecaA.act('c:1', function (err, out) {
            if (!err) assert.fail()
      ...
      setTimeout(function () {
        a.close(function (err) {
          if (err) return fin(err);b.close(function (err) {
            if (err) return fin(err);try {
  ...
  it('message-loop', function (fin) {
    ...
    function do_type (type, fin) {
      ...
      setTimeout(function () {
        a.close(function (err) {
          if (err) return fin(err);b.close(function (err) {
            if (err) return fin(err);c.close(function (err) {
              if (err) return fin(err);try {
                assert.equal(1, counters.a)
                assert.equal(1, counters.b)
