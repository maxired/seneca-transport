...
module.exports = function transport (options) {
  ...
  function json_parser_stream () {
    ...
    json_parser._write = function (data, enc, done) {
      ...
      lines.forEach(function (line) {
        ...
        var jsonstr = self.linebuf.join('')

        self.linebuf.length = 0
  ...
  function hook_listen_web (args, done) {
    ...
    app.use(function (req, res, next) {
      ...
      req.on('end', function () {
        try {
          ...
          var bodydata =
          bufstr.length > 0 ? tu.parseJSON(seneca, 'req-body', bufstr) : {}
    ...
    app.use(function (req, res, next) {
      if (req.url.indexOf(listen_options.path) !== 0) return next()
